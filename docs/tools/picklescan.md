...
**Purpose:** Scan pickle-based ML artifacts for unsafe payloads  
**VEYRA access:** admin  
**Execution boundary:** isolated_analysis_worker

## What is it?
Scan pickle-based ML artifacts for unsafe payloads
...
Use it when the security objective matches **Scan pickle-based ML artifacts for unsafe payloads** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **PickleScan** and read this guide before execution.
...
**One sentence:** PickleScan is used to help the team achieve **Scan pickle-based ML artifacts for unsafe payloads** under an approved and auditable VEYRA workflow.
