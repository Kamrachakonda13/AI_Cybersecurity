...
## What is it?
Kali security utility: google-nexus-tools
...
Use it when the security objective matches **Kali security utility: google-nexus-tools** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **google-nexus-tools** and read this guide before execution.
...
```bash
google-nexus-tools --help
google-nexus-tools --version
```
...
**One sentence:** google-nexus-tools is used to help the team achieve **Kali security utility: google-nexus-tools** under an approved and auditable VEYRA workflow.
