...
Use it when the security objective matches **Vulnerability assessment** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **OpenVAS/Greenbone** and read this guide before execution.
...
```bash
OpenVAS/Greenbone --help
OpenVAS/Greenbone --version
```
...
## Safe workflow
Assess approved hosts, packages, images or applications; de-duplicate findings, map severity/KEV, and track remediation evidence.
...
**One sentence:** OpenVAS/Greenbone is used to help the team achieve **Vulnerability assessment** under an approved and auditable VEYRA workflow.
