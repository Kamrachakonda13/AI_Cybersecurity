...
**Purpose:** Controlled egress for AI workloads  
**VEYRA access:** privileged_admin  
**Execution boundary:** isolated_ai_worker

## What is it?
Controlled egress for AI workloads
...
Use it when the security objective matches **Controlled egress for AI workloads** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **ai-egress-gateway** and read this guide before execution.
...
```bash
ai-egress-gateway --help
ai-egress-gateway --version
```
...
**One sentence:** ai-egress-gateway is used to help the team achieve **Controlled egress for AI workloads** under an approved and auditable VEYRA workflow.
