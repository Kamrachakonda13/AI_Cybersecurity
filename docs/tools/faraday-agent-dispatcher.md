...
## What is it?
Kali security utility: faraday-agent-dispatcher
...
Use it when the security objective matches **Kali security utility: faraday-agent-dispatcher** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **faraday-agent-dispatcher** and read this guide before execution.
...
```bash
faraday-agent-dispatcher --help
faraday-agent-dispatcher --version
```
...
**One sentence:** faraday-agent-dispatcher is used to help the team achieve **Kali security utility: faraday-agent-dispatcher** under an approved and auditable VEYRA workflow.
