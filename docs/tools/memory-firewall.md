...
**Purpose:** Agent memory policy and provenance enforcement  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
Agent memory policy and provenance enforcement
...
Use it when the security objective matches **Agent memory policy and provenance enforcement** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **memory-firewall** and read this guide before execution.
...
**One sentence:** memory-firewall is used to help the team achieve **Agent memory policy and provenance enforcement** under an approved and auditable VEYRA workflow.
