...
Use it when the security objective matches **Metadata extraction** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **ExifTool** and read this guide before execution.
...
## Safe workflow
Extract metadata and recover relevant artifacts from authorized evidence copies without modifying the original evidence.
...
**One sentence:** ExifTool is used to help the team achieve **Metadata extraction** under an approved and auditable VEYRA workflow.
