...
**Category:** AI Gateway  
**Purpose:** LLM request/response policy enforcement integration  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
LLM request/response policy enforcement integration
...
Use it when the security objective matches **LLM request/response policy enforcement integration** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **llm-firewall** and read this guide before execution.
...
**One sentence:** llm-firewall is used to help the team achieve **LLM request/response policy enforcement integration** under an approved and auditable VEYRA workflow.
