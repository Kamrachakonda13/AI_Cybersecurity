...
## What is it?
Kali security utility: mcp-kali-server
...
Use it when the security objective matches **Kali security utility: mcp-kali-server** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **mcp-kali-server** and read this guide before execution.
...
**One sentence:** mcp-kali-server is used to help the team achieve **Kali security utility: mcp-kali-server** under an approved and auditable VEYRA workflow.
