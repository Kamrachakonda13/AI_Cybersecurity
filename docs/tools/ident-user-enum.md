...
## What is it?
Kali security utility: ident-user-enum
...
Use it when the security objective matches **Kali security utility: ident-user-enum** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **ident-user-enum** and read this guide before execution.
...
```bash
ident-user-enum --help
ident-user-enum --version
```
...
**One sentence:** ident-user-enum is used to help the team achieve **Kali security utility: ident-user-enum** under an approved and auditable VEYRA workflow.
