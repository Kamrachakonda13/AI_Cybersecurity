...
**Purpose:** Model registry security and approval workflow  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
Model registry security and approval workflow
...
Use it when the security objective matches **Model registry security and approval workflow** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **model-registry-security** and read this guide before execution.
...
```bash
model-registry-security --help
model-registry-security --version
```
...
**One sentence:** model-registry-security is used to help the team achieve **Model registry security and approval workflow** under an approved and auditable VEYRA workflow.
