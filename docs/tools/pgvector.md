...
**Category:** Vector DB  
**Purpose:** PostgreSQL vector storage integration  
**VEYRA access:** admin  
...
Use it when the security objective matches **PostgreSQL vector storage integration** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **pgvector** and read this guide before execution.
...
**One sentence:** pgvector is used to help the team achieve **PostgreSQL vector storage integration** under an approved and auditable VEYRA workflow.
