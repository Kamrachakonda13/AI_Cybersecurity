...
Use it when the security objective matches **CIS benchmark checks** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **kube-bench** and read this guide before execution.
...
**One sentence:** kube-bench is used to help the team achieve **CIS benchmark checks** under an approved and auditable VEYRA workflow.
