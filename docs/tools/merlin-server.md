...
## What is it?
Kali security utility: merlin-server
...
Use it when the security objective matches **Kali security utility: merlin-server** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **merlin-server** and read this guide before execution.
...
**One sentence:** merlin-server is used to help the team achieve **Kali security utility: merlin-server** under an approved and auditable VEYRA workflow.
