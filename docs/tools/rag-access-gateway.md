...
**Purpose:** Authorization-aware retrieval gateway  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
Authorization-aware retrieval gateway
...
Use it when the security objective matches **Authorization-aware retrieval gateway** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **rag-access-gateway** and read this guide before execution.
...
```bash
rag-access-gateway --help
rag-access-gateway --version
```
...
**One sentence:** rag-access-gateway is used to help the team achieve **Authorization-aware retrieval gateway** under an approved and auditable VEYRA workflow.
