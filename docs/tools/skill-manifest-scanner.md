...
**Purpose:** Agent skill/package manifest security checks  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
Agent skill/package manifest security checks
...
Use it when the security objective matches **Agent skill/package manifest security checks** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **skill-manifest-scanner** and read this guide before execution.
...
```bash
skill-manifest-scanner --help
skill-manifest-scanner --version
```
...
**One sentence:** skill-manifest-scanner is used to help the team achieve **Agent skill/package manifest security checks** under an approved and auditable VEYRA workflow.
