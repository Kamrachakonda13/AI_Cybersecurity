...
**Purpose:** Artifact signing and provenance verification  
**VEYRA access:** admin  
...
Use it when the security objective matches **Artifact signing and provenance verification** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **Cosign/Sigstore** and read this guide before execution.
...
```bash
Cosign/Sigstore --help
Cosign/Sigstore --version
```
...
**One sentence:** Cosign/Sigstore is used to help the team achieve **Artifact signing and provenance verification** under an approved and auditable VEYRA workflow.
