...
Use it when the security objective matches **Portable detection rules** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **Sigma** and read this guide before execution.
...
## Safe workflow
Create portable detections, test against known telemetry, measure false positives and coverage, and version-control the rule.
...
**One sentence:** Sigma is used to help the team achieve **Portable detection rules** under an approved and auditable VEYRA workflow.
