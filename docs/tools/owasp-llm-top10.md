...
**Purpose:** OWASP LLM Top 10 control mapping  
**VEYRA access:** admin  
...
Use it when the security objective matches **OWASP LLM Top 10 control mapping** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **owasp-llm-top10** and read this guide before execution.
...
```bash
owasp-llm-top10 --help
owasp-llm-top10 --version
```
...
**One sentence:** owasp-llm-top10 is used to help the team achieve **OWASP LLM Top 10 control mapping** under an approved and auditable VEYRA workflow.
