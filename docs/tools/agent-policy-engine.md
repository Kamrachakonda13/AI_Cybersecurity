...
**Purpose:** Deterministic agent/tool policy enforcement  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
Deterministic agent/tool policy enforcement
...
Use it when the security objective matches **Deterministic agent/tool policy enforcement** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **agent-policy-engine** and read this guide before execution.
...
```bash
agent-policy-engine --help
agent-policy-engine --version
```
...
**One sentence:** agent-policy-engine is used to help the team achieve **Deterministic agent/tool policy enforcement** under an approved and auditable VEYRA workflow.
