...
**Purpose:** Scan ML model files for unsafe code/components  
**VEYRA access:** admin  
**Execution boundary:** approved_ai_worker

## What is it?
Scan ML model files for unsafe code/components
...
Use it when the security objective matches **Scan ML model files for unsafe code/components** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **protectai-modelscan** and read this guide before execution.
...
```bash
protectai-modelscan --help
protectai-modelscan --version
```
...
**One sentence:** protectai-modelscan is used to help the team achieve **Scan ML model files for unsafe code/components** under an approved and auditable VEYRA workflow.
