...
**Purpose:** Runtime MCP gateway with parameter policies, approvals, secret redaction and audit  
**VEYRA access:** admin  
**Execution boundary:** experimental_worker

## What is it?
Runtime MCP gateway with parameter policies, approvals, secret redaction and audit
...
Use it when the security objective matches **Runtime MCP gateway with parameter policies, approvals, secret redaction and audit** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **MCP Shield Runtime** and read this guide before execution.
...
```bash
MCP Shield Runtime --help
MCP Shield Runtime --version
```
...
**One sentence:** MCP Shield Runtime is used to help the team achieve **Runtime MCP gateway with parameter policies, approvals, secret redaction and audit** under an approved and auditable VEYRA workflow.
