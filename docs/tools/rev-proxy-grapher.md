...
## What is it?
Kali security utility: rev-proxy-grapher
...
Use it when the security objective matches **Kali security utility: rev-proxy-grapher** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **rev-proxy-grapher** and read this guide before execution.
...
```bash
rev-proxy-grapher --help
rev-proxy-grapher --version
```
...
**One sentence:** rev-proxy-grapher is used to help the team achieve **Kali security utility: rev-proxy-grapher** under an approved and auditable VEYRA workflow.
