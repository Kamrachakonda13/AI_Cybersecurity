...
Use it when the security objective matches **Fast port inventory** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **RustScan** and read this guide before execution.
...
**One sentence:** RustScan is used to help the team achieve **Fast port inventory** under an approved and auditable VEYRA workflow.
