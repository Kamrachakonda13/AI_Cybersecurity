...
**Purpose:** Short-lived agent identity and capability broker  
**VEYRA access:** privileged_admin  
**Execution boundary:** isolated_ai_worker

## What is it?
Short-lived agent identity and capability broker
...
Use it when the security objective matches **Short-lived agent identity and capability broker** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **agent-identity-broker** and read this guide before execution.
...
```bash
agent-identity-broker --help
agent-identity-broker --version
```
...
**One sentence:** agent-identity-broker is used to help the team achieve **Short-lived agent identity and capability broker** under an approved and auditable VEYRA workflow.
