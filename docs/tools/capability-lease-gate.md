...
**Purpose:** Invocation-scoped capability lease enforcement  
**VEYRA access:** privileged_admin  
**Execution boundary:** isolated_ai_worker

## What is it?
Invocation-scoped capability lease enforcement
...
Use it when the security objective matches **Invocation-scoped capability lease enforcement** and the target is owned, explicitly authorized, and inside the recorded scope.

## VEYRA UI workflow
1. Open **Tool Runner / Sudo Security Arsenal**.
2. Search for **capability-lease-gate** and read this guide before execution.
...
```bash
capability-lease-gate --help
capability-lease-gate --version
```
...
**One sentence:** capability-lease-gate is used to help the team achieve **Invocation-scoped capability lease enforcement** under an approved and auditable VEYRA workflow.
