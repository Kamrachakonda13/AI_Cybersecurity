...
ai_ecosystem) and renders:
    - one markdown file per tool to docs/tools/<id>.md
    - a browsable index at docs/tools/README.md
...
def render_index(tools: dict[str, dict]) -> str:
    """Render docs/tools/README.md — a browsable index grouped by category."""
    by_cat: dict[str, list[tuple[str, str, str]]] = {}
    for tid, tool in tools.items():
        cat = tool.get("category", "Uncategorized")
        name = tool.get("name", tid)
        purpose = tool.get("purpose", "")
        by_cat.setdefault(cat, []).append((name, tid, purpose))
    ...
    lines: list[str] = [
        ...
        "",
        f"**{total} tools** across **{cat_count} categories**. Auto-generated from "
        "the VEYRA catalog by `scripts/generate_tool_docs.py`.",
        "",
        "> **Do not edit this file by hand.** It is regenerated automatically. "
        "To add or change a tool, update the catalog and run "
    ...
    for cat in sorted(by_cat.keys()):
        ...
        lines.append(f"## {cat} ({len(entries)} tool{plural})")
        lines.append("")
        for name, tid, purpose in entries:
            # Guard against pipe characters breaking table syntax if we
            # ever change format; keep bullets safe too.
            purpose_safe = purpose.replace("|", "\\|")
            lines.append(f"- [{name}]({tid}.md) — {purpose_safe}")
        lines.append("")
...
def main() -> int:
    ...
    # --- Index ---
    rendered_index = render_index(tools)
    if INDEX_FILE.exists():
        index_changed = INDEX_FILE.read_text(
            encoding="utf-8") != rendered_index
    else:
        index_changed = True

    if index_changed and not args.check:
        INDEX_FILE.write_text(rendered_index, encoding="utf-8")
    ...
    print(f"Index:           {'changed' if index_changed else 'up-to-date'}"
          f"  ({INDEX_FILE.relative_to(REPO)})")

    if args.check and (missing or changed or index_changed):
        print()
