...
from ..models import Asset, Service, Finding, Incident, AuditEvent, Identity, SessionEvent, NetworkFlow, ThreatIntel, CloudResource, AIAsset, SecurityToolJob, SecurityEvidence, AgentRuntimeEvent, AgentPolicy, UnifiedSecurityEvent, PentestAgentPlan, WifiNetwork, UserAccount, UserToolPermission, UserSession, ToolAccessRequest, AdminNotification, SecureCodeShare, WorkerNode, WorkerToolInstall, SupplyChainAttestation, ToolCanaryCohort, AISupplyChainAsset, AgentCircuitBreaker
...
def require_privileged_admin(request: Request):
    ...
    if not expected or not supplied or supplied != expected:
        raise HTTPException(
            403, "Privileged administrator authorization required")
    return True
    ...
    if user is not None:
        ...
        try:
            ...
            if level not in {"plan", "execute_request"}:
                raise HTTPException(
                    403, "This tool is read-only for your account. Use Request access to ask the sudo administrator.")
            if tool.get("access_tier") == "privileged_admin" and user.role not in {"sudo", "security_admin"}:
                raise HTTPException(
                    403, "Privileged tools require Sudo or Security Admin role")
        finally:
            db.close()
...
def stage_ethical_job(req: EthicalJobRequest, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if not ticket:
        if user is not None and user.role == "sudo":
            ...
            raise HTTPException(
                400, "approval_ticket is required (sudo runs generate one automatically)")
    try:
        _, _, extra = build_contract_fields(
            tool, {"target": req.target, "scope": req.scope, **(req.params or {})})
    except ValueError as e:
        raise HTTPException(400, str(e))
    merged_params = {**(req.params or {}), **extra}
    try:
        contract = make_job_contract(
            tool, req.target, req.scope, ticket, req.environment, req.purpose, actor=actor)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def admin_ethical_jobs(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    rows = db.query(SecurityToolJob).order_by(
        SecurityToolJob.created_at.desc()).limit(limit).all()
    out = []
    for r in rows:
        item = clean(r)
        item["scope"] = json.loads(r.scope or "[]")
        try:
            item["params"] = json.loads(r.params or "{}")
        except Exception:
            item["params"] = {}
        out.append(item)
    return out
...
def approve_ethical_job(job_id: str, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    row = db.query(SecurityToolJob).filter(
        SecurityToolJob.job_id == job_id).first()
    if not row:
        raise HTTPException(404, "Job not found")
    if row.status != "pending_approval":
        raise HTTPException(409, f"Job is {row.status}")
    tool = next((x for x in admin_tool_registry()
                if x["name"] == row.tool), None)
    if tool and tool.get("access_tier") == "privileged_admin":
        require_privileged_admin(request)
    from datetime import datetime, timezone
    row.status = "approved_for_worker"
    row.approved_at = datetime.now(timezone.utc)
    db.add(AuditEvent(actor="admin", action="ethical_hacking_job_approved",
           target=job_id, outcome="approved_for_worker"))
    db.commit()
...
def dispatch_ethical_job(job_id: str, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if not row:
        raise HTTPException(404, "Job not found")
    if row.status != "approved_for_worker":
        raise HTTPException(409, "Job must be approved before dispatch")
    tool = next((x for x in admin_tool_registry()
                if x["name"] == row.tool), None)
    if tool and tool.get("access_tier") == "privileged_admin":
        require_privileged_admin(request)
    row.status = "queued_for_isolated_worker"
    db.add(AuditEvent(actor="admin", action="ethical_hacking_job_dispatched",
           target=job_id, outcome="queued"))
    db.commit()
...
def ingest_ethical_evidence(job_id: str, req: EvidenceRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if not row:
        raise HTTPException(404, "Job not found")
    try:
        artifact = normalize_evidence(job_id, req.payload)
    except ValueError as e:
        raise HTTPException(400, str(e))
    ...
    db.add(AuditEvent(actor="admin", action="ethical_hacking_evidence_ingested",
           target=job_id, outcome=artifact["artifact_id"]))
    db.commit()
    return artifact
...
def list_ethical_evidence(job_id: str, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    rows = db.query(SecurityEvidence).filter(SecurityEvidence.job_id ==
                                             job_id).order_by(SecurityEvidence.created_at.desc()).all()
    out = []
    for r in rows:
        item = clean(r)
        item["data"] = json.loads(r.data or "{}")
        out.append(item)
    return out
...
def run_ethical_job_now(req: EthicalJobRequest, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    ticket = req.approval_ticket.strip() or (
        "SUDO-" + datetime.now(_tz.utc).strftime("%Y%m%d-%H%M%S"))
    try:
        _, _, extra = build_contract_fields(
            tool, {"target": req.target, "scope": req.scope, **(req.params or {})})
    except ValueError as e:
    ...
    db.add(row)
    db.add(AuditEvent(actor=actor, action="ethical_hacking_job_staged",
           target=req.target, outcome="sudo-run-queued"))
    db.add(AuditEvent(actor=actor, action="ethical_hacking_job_approved",
           target=contract["job_id"], outcome="sudo_implicit_approval"))
    db.add(AuditEvent(actor=actor, action="ethical_hacking_job_dispatched",
           target=contract["job_id"], outcome="queued"))
    db.commit()
...
def create_pentest_agent_plan(req: PentestPlanRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        plan = create_plan(req.agent_id, req.objective, req.target,
                           req.scope, req.environment, req.approval_ticket)
    except ValueError as e:
        raise HTTPException(400, str(e))
    row = PentestAgentPlan(plan_id=plan["plan_id"], agent_id=plan["agent_id"], objective=plan["objective"], target=plan["target"], scope_json=json.dumps(
        plan["scope"]), environment=plan["environment"], approval_ticket=plan["approval_ticket"], chain_json=json.dumps(plan["chain"]), status=plan["status"], plan_sha256=plan["plan_sha256"])
    db.add(row)
    db.add(AuditEvent(actor="admin", action="pentest_agent_plan_created",
           target=req.target, outcome=plan["plan_id"]))
    db.commit()
    return plan
...
def list_pentest_agent_plans(limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    rows = db.query(PentestAgentPlan).order_by(
        PentestAgentPlan.created_at.desc()).limit(limit).all()
...
def get_agent_policy(agent_id: str, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    row = db.query(AgentPolicy).filter(
        AgentPolicy.agent_id == agent_id).first()
    if not row:
...
def put_agent_policy(req: AgentPolicyRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    row = db.query(AgentPolicy).filter(
        AgentPolicy.agent_id == req.agent_id).first()
    if not row:
        row = AgentPolicy(agent_id=req.agent_id)
        db.add(row)
    ...
    db.add(AuditEvent(actor="admin", action="ai_agent_policy_updated",
           target=req.agent_id, outcome="success"))
    db.commit()
...
def ai_gateway_evaluate(req: AIGatewayRequest, db: Session = Depends(get_db), _: bool = Depends(require_ai_gateway)):
    ...
    row = db.query(AgentPolicy).filter(AgentPolicy.agent_id ==
                                       req.agent_id, AgentPolicy.enabled.is_(True)).first()
    if not row:
        raise HTTPException(
            403, "Agent is not enrolled in the VEYRA AI gateway")
    policy = {"allowed_tools": json.loads(row.allowed_tools or "[]"), "allowed_operations": json.loads(
        row.allowed_operations or "[]"), "max_risk_score": row.max_risk_score, "require_human_approval": row.require_human_approval}
    decision = decide(req.model_dump(), policy)
    tid = trace_id()
    event = {"trace_id": tid, "agent_id": req.agent_id, "operation": req.operation, "provider": req.provider,
             "model": req.model, "tool_name": req.tool_name, "decision": decision, "metadata": req.metadata, "timestamp": utc()}
    event["event_sha256"] = event_hash(event)
    db.add(AgentRuntimeEvent(trace_id=tid, agent_id=req.agent_id, operation=req.operation, provider=req.provider, model=req.model,
           tool_name=req.tool_name, policy_decision=decision["decision"], risk_score=decision["risk_score"], event_json=json.dumps(event, default=str)))
    db.add(AuditEvent(actor=req.agent_id, action="ai_gateway_decision",
           target=req.operation, outcome=decision["decision"]))
    db.commit()
...
def ai_gateway_trace(trace_id: str, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    rows = db.query(AgentRuntimeEvent).filter(AgentRuntimeEvent.trace_id ==
                                              trace_id).order_by(AgentRuntimeEvent.created_at.asc()).all()
...
def ai_gateway_events(limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    rows = db.query(AgentRuntimeEvent).order_by(
        AgentRuntimeEvent.created_at.desc()).limit(limit).all()
    return [clean(r) | {"event": json.loads(r.event_json or "{}")} for r in rows]
...
def fabric_event(req: FabricEventRequest, db: Session = Depends(get_db), _: bool = Depends(require_collector)):
    ...
    row = UnifiedSecurityEvent(event_id=e["event_id"], trace_id=e["trace_id"], plane=e["plane"], event_type=e["event_type"], actor=e["actor"], source=e["source"],
                               target=e["target"], risk_score=e["risk_score"], severity=e["severity"], payload=json.dumps(e["payload"], default=str), event_sha256=e["event_sha256"])
    db.add(row)
    db.add(AuditEvent(actor=req.actor or "fabric", action="fabric_event_ingested",
           target=req.target or req.event_type, outcome=e["severity"]))
    db.commit()
    return e
...
def fabric_events(limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    rows = db.query(UnifiedSecurityEvent).order_by(
        UnifiedSecurityEvent.observed_at.desc()).limit(limit).all()
...
def ports(db: Session = Depends(get_db)):
    rows = db.query(Service, Asset).join(
        Asset, Service.asset_id == Asset.id).all()
...
def flows(db: Session = Depends(get_db)):
    rows = db.query(NetworkFlow).order_by(
        NetworkFlow.observed_at.desc()).limit(100).all()
    return [clean(x) for x in rows]


@router.get("/findings")
def findings(db: Session = Depends(get_db)):
    rows = db.query(Finding, Asset).join(Asset, Finding.asset_id ==
                                         Asset.id).order_by(Finding.risk_score.desc()).all()
...
def investigate(req: InvestigationRequest, db: Session = Depends(get_db)):
    ...
    if req.asset_id:
        ...
        if a:
            context.append(
                {"asset": a.hostname, "ip": a.ip_address, "criticality": a.criticality})
    f = db.query(Finding).order_by(Finding.risk_score.desc()).limit(5).all()
    ti = db.query(ThreatIntel).filter(
        ThreatIntel.exploited.is_(True)).limit(5).all()
...
def create_assessment(req: AssessmentRequest, db: Session = Depends(get_db)):
    if not req.scope or not req.target.strip():
        raise HTTPException(400, "Explicit target and scope are required")
    db.add(AuditEvent(actor="demo-user", action="assessment_created", target=req.target,
           outcome="pending-approval" if req.approval_required else "queued"))
    db.commit()
...
def ingest_flows(items: list[FlowIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    if not items or len(items) > 500:
        raise HTTPException(400, "Send 1..500 flows per batch")
    rows = [NetworkFlow(**i.model_dump()) for i in items]
    db.add_all(rows)
    db.add(AuditEvent(actor="collector", action="ingest_network_flows",
           target=f"{len(rows)} flows", outcome="success"))
    db.commit()
...
def ingest_sessions(items: list[SessionIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    if not items or len(items) > 500:
        raise HTTPException(400, "Send 1..500 sessions per batch")
    rows = [SessionEvent(**i.model_dump()) for i in items]
    db.add_all(rows)
    db.add(AuditEvent(actor="collector", action="ingest_sessions",
           target=f"{len(rows)} sessions", outcome="success"))
    db.commit()
    return {"ingested": len(rows)}
...
def ingest_services(items: list[ServiceIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    if not items or len(items) > 200:
        raise HTTPException(400, "Send 1..200 services per batch")
    for i in items:
        if not db.get(Asset, i.asset_id):
            raise HTTPException(400, f"Unknown asset_id {i.asset_id}")
    rows = [Service(**i.model_dump()) for i in items]
    db.add_all(rows)
    db.add(AuditEvent(actor="collector", action="ingest_services",
           target=f"{len(rows)} services", outcome="success"))
    db.commit()
    return {"ingested": len(rows)}
...
async def web_assessment(req: WebAssessmentRequest, db: Session = Depends(get_db)):
    ...
    if info["private"] and not req.approval_confirmed:
        db.add(AuditEvent(actor="demo-user", action="web_assessment_pending_approval",
               target=req.url, outcome="pending-approval"))
        db.commit()
        lab_hint = " Lab target detected — get red-team lead approval, then resubmit." if info.get(
            "lab") else ""
    ...
    try:
        ...
        db.add(AuditEvent(actor="demo-user",
               action="web_assessment_failed", target=req.url, outcome="error"))
        db.commit()
    ...
    db.add(AuditEvent(actor="demo-user", action="web_assessment_completed",
           target=req.url, outcome="success"))
    db.commit()
...
async def forensics_analyze(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if len(data) > MAX_BYTES:
        raise HTTPException(
            413, "File too large — 5 MB cap for safe static analysis")
    if not data:
...
def ingest_devices(items: list[DeviceIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    if not items or len(items) > 500:
        raise HTTPException(400, "Send 1..500 devices per batch")
    from datetime import datetime, timezone
    ...
    for i in items:
        ...
        if d:
            d.ip_address = i.ip_address or d.ip_address
            d.mac = i.mac or d.mac
            d.os = i.os or d.os
            d.owner = i.owner or d.owner
            d.last_seen = datetime.now(timezone.utc)
        else:
            db.add(Device(hostname=host, ip_address=i.ip_address, mac=i.mac,
                          os=i.os, owner=i.owner))
        n += 1
    db.add(AuditEvent(actor="collector", action="ingest_devices",
           target=f"{n} devices", outcome="success"))
    db.commit()
    return {"ingested": n}
...
def ingest_dns(items: list[DnsIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    if not items or len(items) > 1000:
        raise HTTPException(400, "Send 1..1000 queries per batch")
    ...
    if any(not r.domain for r in rows):
        raise HTTPException(400, "domain required")
    db.add_all(rows)
    db.add(AuditEvent(actor="collector", action="ingest_dns",
           target=f"{len(rows)} queries", outcome="success"))
    db.commit()
    return {"ingested": len(rows)}
...
def ingest_logins(items: list[LoginIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    if not items or len(items) > 1000:
        raise HTTPException(400, "Send 1..1000 attempts per batch")
...
def session_triage(req: TriageRequest, db: Session = Depends(get_db)):
    ...
    if not req.ip.strip() and not req.username.strip():
        raise HTTPException(
            400, "Provide ip and/or username from your own systems")
    ...
    identity = clean(db.query(Identity).filter(
        Identity.username == user).first()) if user else None
    ...
    if fails >= 5:
        steps.insert(
            1, f"Brute-force candidate ({fails} fails): prioritize lockout + alerting per policy.")
...
_STORAGE_HINTS = ("mass storage", "disk", "flash",
                  "thumb", "usb drive", "external")
    ...
    for i in items:
        if i.action == "connect" and any(h in i.device.lower() for h in _STORAGE_HINTS):
            asset = db.query(Asset).filter(
                Asset.hostname == i.hostname.strip()).first()
            if asset and asset.criticality >= 5:
                db.add(Finding(asset_id=asset.id,
                               title=f"USB mass storage on critical host {i.hostname}"[
                                   :255],
...
def ingest_dlp(items: list[DlpIn], db: Session = Depends(get_db),
    ...
    for i in items:
        ...
        if i.action not in ("blocked", "quarantined", "allowed"):
            raise HTTPException(
                400, "action must be blocked|quarantined|allowed")
    db.add_all([DlpEvent(**i.model_dump()) for i in items])
    auto = 0
    for i in items:
        if i.classification in ("secret", "restricted") and i.action == "allowed":
            db.add(Finding(asset_id=db.query(Asset).first().id if db.query(Asset).first() else 1,
                           title=f"DLP policy gap: {i.classification} left boundary ({i.filepath})"[
                :255],
...
def cloud_prowler(items: list[ProwlerItem], db: Session = Depends(get_db)):
    ...
    if not items or len(items) > 1000:
        raise HTTPException(400, "Send 1..1000 items per batch")
    out = import_prowler(db, [i.model_dump() for i in items])
    db.add(AuditEvent(actor="analyst", action="prowler_import",
           target=str(out), outcome="success"))
    db.commit()
    return out
...
def cloud_scan(req: CloudScanRequest, db: Session = Depends(get_db)):
    ...
    if req.provider != "aws":
        raise HTTPException(
            400, "Only 'aws' live scan is implemented; use import-prowler for the rest")
    try:
        import boto3  # type: ignore
    except ImportError:
        raise HTTPException(
            501, "boto3 not installed: pip install boto3 + configure read-only AWS creds, then retry")
    if not (_os.getenv("AWS_ACCESS_KEY_ID") and _os.getenv("AWS_SECRET_ACCESS_KEY")):
        raise HTTPException(
            501, "AWS creds absent: set AWS_ACCESS_KEY_ID/SECRET_ACCESS_KEY (read-only IAM) on the backend host")
    from ..services.cloud_adapters import aws_live_scan
    out = aws_live_scan(db, boto3.client(
        "ec2"), boto3.client("s3"), boto3.client("iam"))
    db.add(AuditEvent(actor="analyst", action="aws_live_scan",
           target=str(out), outcome="success"))
    db.commit()
    return out
...
async def ai_evaluate(req: AiEvalRequest, db: Session = Depends(get_db)):
    ...
    if not (info.get("lab") or info["private"]):
        raise HTTPException(
            400, "AI evaluation is restricted to lab/private targets — internet AI is out of scope")
    ...
    if report["failed"]:
        db.add(Finding(asset_id=db.query(Asset).first().id if db.query(Asset).first() else 1,
                       title=f"AI gateway complied with {report['failed']}/{len(report['probes'])} hostile probes"[
            :255],
...
def ingest_discovery(items: list[DiscoveryIn], db: Session = Depends(get_db),
    ...
    if not items or len(items) > 1000:
        raise HTTPException(400, "Send 1..1000 hosts per batch")
...
def ingest_wifi_networks(items: list[WifiNetworkIn], db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    ...
    for i in items:
        ...
        if not key or key in seen:
            continue
        seen.add(key)
        row = db.query(WifiNetwork).filter(
            WifiNetwork.bssid == bssid).first() if bssid else None
        if row is None:
            row = WifiNetwork(ssid=i.ssid[:255], bssid=bssid[:64], security=i.security[:64],
                              channel=i.channel[:32], band=i.band[:32], signal_dbm=i.signal_dbm, source=i.source[:64])
            db.add(row)
        else:
            row.ssid = i.ssid[:255]
            row.security = i.security[:64]
            row.channel = i.channel[:32]
            row.band = i.band[:32]
            row.signal_dbm = i.signal_dbm
            row.source = i.source[:64]
            row.observed_at = datetime.now(timezone.utc)
    db.add(AuditEvent(actor="wifi-sensor", action="ingest_wifi_scan",
           target=f"{len(seen)} networks", outcome="success"))
    db.commit()
...
def update_discovered_host_trust(host_id: int, req: WifiTrustUpdate, db: Session = Depends(get_db)):
    ...
    db.add(AuditEvent(actor="console-user", action="wifi_device_trust_changed",
           target=f"{row.mac or row.ip_address}", outcome="trusted" if req.trusted else "untrusted"))
    db.commit()
    return clean(row)
...
def discovery_wifi_networks(limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db)):
    rows = db.query(WifiNetwork).order_by(WifiNetwork.signal_dbm.desc(
    ).nullslast(), WifiNetwork.observed_at.desc()).limit(limit).all()
    return [clean(x) for x in rows]
...
def discovery_summary(db: Session = Depends(get_db)):
    ...
    named = 0
    for r in rows:
        by_vendor[r.vendor or "Unknown"] = by_vendor.get(
            r.vendor or "Unknown", 0) + 1
...
def worker_next_job(db: Session = Depends(get_db), _: bool = Depends(require_worker)):
    from ..services.execution_plane import make_job_contract
    row = db.query(SecurityToolJob).filter(SecurityToolJob.status ==
                                           "queued_for_isolated_worker").order_by(SecurityToolJob.created_at.asc()).first()
    if not row:
        return {"job": None}
    tools = {x["name"]: x for x in admin_tool_registry()}
    tool = tools.get(row.tool)
    if not tool:
        raise HTTPException(409, "Registered tool definition missing")
    contract = {"job_id": row.job_id, "tool": row.tool, "tool_id": tool["id"], "execution_profile": tool["execution_profile"], "target": row.target, "scope": json.loads(
        row.scope or "[]"), "approval_ticket": row.approval_ticket, "environment": row.environment, "purpose": row.purpose, "actor": row.actor, "created_at": row.created_at.isoformat() if row.created_at else "", "execution": "not_started", "browser_shell": False, "contract_sha256": row.contract_sha256}
    from ..services.execution_plane import hmac, hashlib
    secret = _os.getenv("VEYRA_WORKER_SIGNING_SECRET", "")
    if not secret:
        raise HTTPException(503, "Worker signing secret is not configured")
    contract["contract_signature"] = hmac.new(
        secret.encode(), row.contract_sha256.encode(), hashlib.sha256).hexdigest()
    row.status = "worker_claimed"
    db.add(AuditEvent(actor="isolated-worker", action="ethical_hacking_job_claimed",
           target=row.job_id, outcome="worker_claimed"))
    db.commit()
    return {"job": contract}
...
def worker_evidence(req: WorkerEvidenceRequest, db: Session = Depends(get_db), _: bool = Depends(require_worker)):
    ...
    row = db.query(SecurityToolJob).filter(
        SecurityToolJob.contract_sha256 == req.contract_sha256).first()
    ...
    try:
        artifact = normalize_evidence(row.job_id, req.payload)
    except ValueError as e:
        raise HTTPException(400, str(e))
    row.status = "completed"
    db.add(SecurityEvidence(artifact_id=artifact["artifact_id"], job_id=row.job_id, sha256=artifact["sha256"], source=artifact["source"], collector=artifact["collector"],
           collected_at=artifact["collected_at"], classification=artifact["classification"], result_type=artifact["result_type"], summary=artifact["summary"], data=json.dumps(artifact["data"], default=str)))
    db.add(AuditEvent(actor="isolated-worker", action="ethical_hacking_evidence_ingested",
           target=row.job_id, outcome=artifact["artifact_id"]))
    db.commit()
...
def autonomous_soc_investigate(req: InvestigationStartRequest, db: Session = Depends(get_db)):
    ...
    try:
        return investigate_event(db, req.event_id)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def autonomous_soc_containment(case_id: str, req: ContainmentRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return request_containment(db, case_id, req.action, req.target, req.reason)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def autonomous_soc_approval(approval_id: int, req: ApprovalDecisionRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return decide_approval(db, approval_id, req.approved, req.actor)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def detection_mesh_alert(req: MeshAlertRequest, db: Session = Depends(get_db), _: bool = Depends(require_collector)):
    ...
    try:
        return ingest_alert(db, req.model_dump())
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def detection_mesh_response(case_id: str, req: MeshResponseRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return request_response(db, case_id, req.action, req.target, req.approval_id)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def detection_mesh_verify(action_id: str, req: MeshVerifyRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return verify_response(db, action_id, req.observed_state, req.evidence)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def intel_fusion_hypotheses(limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    rows = db.query(AttributionHypothesis).order_by(
        AttributionHypothesis.confidence.desc()).limit(limit).all()
...
def intel_fusion_enrichments(limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    rows = db.query(IntelEnrichment).order_by(
        IntelEnrichment.created_at.desc()).limit(limit).all()
...
def v29_evidence_bundle(db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    db.add(AuditEvent(actor="v29-console", action="evidence_bundle_staged",
           target=bundle["bundle_id"], outcome="staged"))
    db.commit()
    return bundle
...
VALID_ROLES = {"sudo", "security_admin",
               "security_operator", "analyst", "viewer"}
...
def _validate_permissions(perms: dict[str, str]):
    ...
    if unknown:
        raise HTTPException(
            400, f"Unknown tool IDs: {', '.join(unknown[:10])}")
    if bad:
        raise HTTPException(400, f"Invalid permission levels: {bad}")
    return levels
...
def auth_login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    user = db.query(UserAccount).filter(
        UserAccount.username == req.username).first()
    ...
    db.add(AuditEvent(actor=user.username, action="login",
           target=user.username, outcome="success"))
    db.commit()
...
def auth_mfa_verify(req: MfaVerifyRequest, db: Session = Depends(get_db)):
    ...
    if not result:
        raise HTTPException(
            401, "Invalid OTP or MFA challenge expired/exhausted")
    user = db.query(UserAccount).filter(
        UserAccount.username == req.username).first()
    token = issue_session(db, user)
    db.add(AuditEvent(actor=user.username, action="login",
...
def auth_logout(request: Request, db: Session = Depends(get_db)):
    ...
    import hashlib
    s = db.query(UserSession).filter(UserSession.token_hash ==
                                     hashlib.sha256(raw.encode()).hexdigest()).first()
    if s:
        s.revoked_at = __import__('datetime').datetime.now(
            __import__('datetime').timezone.utc)
    db.add(AuditEvent(actor=user.username, action="logout",
           target=user.username, outcome="success"))
    db.commit()
...
def auth_password(req: PasswordChangeRequest, request: Request, db: Session = Depends(get_db)):
    ...
    db.add(AuditEvent(actor=user.username, action="password_changed",
           target=user.username, outcome="success"))
    db.commit()
    return {"status": "password_changed"}


class SecureCodeShareRequest(BaseModel):
    code: str = Field(min_length=1, max_length=4096)
    recipient_hint: str = Field(default="", max_length=255)
    expires_in_minutes: int = Field(default=10, ge=1, le=60)


def _secure_share_cipher():
    import base64
    import hashlib
    from cryptography.fernet import Fernet
    seed = _os.getenv("VEYRA_SECURE_SHARE_KEY") or _os.getenv(
        "VEYRA_ADMIN_TOKEN") or "local-development-secure-share-key"
    key = base64.urlsafe_b64encode(hashlib.sha256(seed.encode()).digest())
    return Fernet(key)


@router.post("/admin/secure-code-shares")
def create_secure_code_share(req: SecureCodeShareRequest, request: Request, db: Session = Depends(get_db)):
    ...
    expires_at = datetime.now(timezone.utc) + \
        timedelta(minutes=req.expires_in_minutes)
    row = SecureCodeShare(token_hash=token_hash, code_ciphertext=_secure_share_cipher().encrypt(
        req.code.encode()).decode(), recipient_hint=req.recipient_hint, created_by=actor.username, expires_at=expires_at)
    db.add(row)
    db.add(AuditEvent(actor=actor.username, action="secure_code_share_created",
                      target=req.recipient_hint or "one-time-share",
                      outcome=f"expires:{req.expires_in_minutes}m"))
    db.commit()
    base = _os.getenv("VEYRA_PUBLIC_APP_URL",
                      "http://localhost:3000").rstrip("/")
    return {"url": f"{base}/secure-code/{raw_token}", "expires_at": expires_at.isoformat(), "recipient_hint": req.recipient_hint}


@router.get("/auth/secure-code-shares/{token}")
def redeem_secure_code_share(token: str, db: Session = Depends(get_db)):
    import hashlib
    from datetime import datetime, timezone
    if len(token) < 40:
        raise HTTPException(404, "Secure share unavailable")
    now = datetime.now(timezone.utc)
    row = db.query(SecureCodeShare).filter(SecureCodeShare.token_hash == hashlib.sha256(token.encode()).hexdigest(
    ), SecureCodeShare.consumed_at.is_(None), SecureCodeShare.expires_at > now).with_for_update().first()
    if not row:
        raise HTTPException(
            404, "Secure share unavailable, expired, or already consumed")
    code = _secure_share_cipher().decrypt(row.code_ciphertext.encode()).decode()
    row.consumed_at = now
    db.delete(row)
    db.add(AuditEvent(actor="secure-share-recipient",
           action="secure_code_share_consumed", target="one-time-share", outcome="destroyed"))
    db.commit()
    return {"code": code, "recipient_hint": row.recipient_hint, "consumed": True}
...
def users_list(request: Request, db: Session = Depends(get_db)):
    ...
    out = []
    for u in rows:
        perms = db.query(UserToolPermission).filter(
            UserToolPermission.user_id == u.id).all()
        grants = {}
        for p in perms:
            grants[p.tool_id] = {"level": p.level,
                                 "expires_at": p.expires_at.isoformat() if p.expires_at else None,
                                 "granted_by": p.granted_by}
        out.append({"id": u.id, "username": u.username, "display_name": u.display_name, "email": u.email, "role": u.role, "status": u.status,
                   "mfa_required": u.mfa_required, "must_change_password": u.must_change_password, "tool_permissions": {p.tool_id: p.level for p in perms}, "tool_grants": grants})
    return out
...
def users_tool_catalog(request: Request, db: Session = Depends(get_db)):
    ...
    return {"tools": admin_tool_registry(), "permission_levels": [
        {"id": "none", "label": "No access",
            "description": "Tool is hidden from the user's usable tool list."},
        {"id": "view", "label": "View",
            "description": "Can read catalog/help and evidence; cannot stage work."},
        {"id": "plan", "label": "Plan",
            "description": "Can create a governed assessment plan; approval remains required."},
        {"id": "execute_request", "label": "Request execution",
            "description": "Can submit an execution request; worker execution still requires governance/approval."}
    ]}
...
def users_create(req: UserCreateRequest, request: Request, db: Session = Depends(get_db)):
    ...
    u = UserAccount(username=req.username, display_name=req.display_name, email=req.email, password_hash=hash_password(
        req.password), role=req.role, mfa_required=req.mfa_required, must_change_password=True)
    db.add(u)
    db.flush()
    for tool_id, raw in req.tool_permissions.items():
        level, expires_at = _grant_expiry_from_value(raw)
        if level != "none":
            db.add(UserToolPermission(user_id=u.id, tool_id=tool_id,
                   level=level, granted_by=actor.username, expires_at=expires_at))
    db.add(AuditEvent(actor=actor.username, action="user_created",
           target=req.username, outcome=req.role))
    db.commit()
...
def users_update(user_id: int, req: UserUpdateRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if req.role is not None:
        if req.role not in VALID_ROLES:
            raise HTTPException(400, "Invalid role")
        if req.role == "sudo" and u.role != "sudo" and db.query(UserAccount).filter(UserAccount.role == "sudo").count():
            raise HTTPException(409, "VEYRA allows exactly one sudo account")
        u.role = req.role
    ...
    if req.status is not None:
        ...
        if u.role == "sudo" and req.status != "active":
            raise HTTPException(
                400, "The sole sudo account cannot be disabled")
        u.status = req.status
    if req.mfa_required is not None:
        u.mfa_required = req.mfa_required
    if req.tool_permissions is not None:
        levels = _validate_permissions(req.tool_permissions)
        db.query(UserToolPermission).filter(
            UserToolPermission.user_id == u.id).delete(synchronize_session=False)
        for tool_id, raw in req.tool_permissions.items():
            level, expires_at = _grant_expiry_from_value(raw)
            if level != "none":
                db.add(UserToolPermission(user_id=u.id, tool_id=tool_id,
                       level=level, granted_by=actor.username, expires_at=expires_at))
    db.add(AuditEvent(actor=actor.username,
           action="user_permissions_updated", target=u.username, outcome=u.role))
    db.commit()
...
def my_tool_permissions(request: Request, db: Session = Depends(get_db)):
    ...
    user = _auth_user(request, db)
    perms = {p.tool_id: p.level for p in db.query(UserToolPermission).filter(
        UserToolPermission.user_id == user.id).all()}
    grants = {}
    for p in db.query(UserToolPermission).filter(UserToolPermission.user_id == user.id).all():
        ...
        if p.expires_at is not None:
            try:
                exp = p.expires_at.replace(
                    tzinfo=p.expires_at.tzinfo) if False else exp
            except Exception:
                pass
        grants[p.tool_id] = {"level": p.level,
                             "expires_at": exp, "granted_by": p.granted_by}
    if user.role == "sudo":
    ...
    tools = [t for t in admin_tool_registry(
    ) if perms.get(t["id"], "none") != "none"]
...
def _sudo_inbox_targets(db: Session):
    ...
    emails = {u.email for u in db.query(UserAccount).filter(
        UserAccount.role == "sudo", UserAccount.status == "active").all() if u.email}
...
def tool_access_request(req: AccessRequestIn, request: Request, db: Session = Depends(get_db)):
    ...
    if user.role == "sudo":
        raise HTTPException(
            400, "Sudo already holds every tool; no request needed")
    ...
    if dup:
        raise HTTPException(
            409, "You already have a pending request for this tool")
...
def tool_access_decide(request_id: str, req: AccessDecideIn, request: Request, db: Session = Depends(get_db)):
    ...
    from ..services.notify import send_access_request_email
    actor = _sudo(request, db)
    row = db.query(ToolAccessRequest).filter(
        ToolAccessRequest.request_id == request_id).first()
    if not row:
        raise HTTPException(404, "Request not found")
    if row.status != "pending":
        raise HTTPException(409, f"Request is {row.status}")
    if req.approve:
        ...
        if not grant:
            grant = UserToolPermission(
                user_id=row.user_id, tool_id=row.tool_id)
            db.add(grant)
        ...
        row.status = "approved"
        row.decided_by = actor.username
        row.decided_at = datetime.now(_tz.utc)
...
def admin_notifications(limit: int = Query(50, ge=1, le=200), request: Request = None, db: Session = Depends(get_db)):
    ...
    if user.role != "sudo":
        my_ids = [r.request_id for r in db.query(ToolAccessRequest).filter(
            ToolAccessRequest.user_id == user.id).all()]
        q = q.filter(AdminNotification.to_role == "user",
                     AdminNotification.related_id.in_(my_ids) if my_ids else False)
...
def tool_install_plan(tool_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    if user is None:
        raise HTTPException(401, "Authenticated console session required")
    tool = next((x for x in admin_tool_registry() +
                ai_ecosystem_registry() if x["id"] == tool_id), None)
    if not tool:
        raise HTTPException(404, "Tool is not registered")
    if tool.get("privileged_usage") and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Privileged tool installation requires Sudo or Security Admin")
...
def tool_install_request(req: dict, request: Request, db: Session = Depends(get_db)):
    ...
    if user.role != "sudo" and not ticket:
        raise HTTPException(
            400, "approval_ticket is required for non-sudo installation requests")
...
def security_tool_academy():
    ...
    for t in admin_tool_registry():
        x = dict(t)
        x["learning_guide"] = guidance.get(
            t["category"], "Define explicit scope, use an approved worker or connector, review evidence, and record provenance before taking action.")
        x["recommended_learning_order"] = ["Understand purpose", "Define authorized scope",
                                           "Run/observe in approved environment", "Review evidence", "Map findings to risk", "Remediate and verify"]
        out.append(x)
...
def register_worker(req: WorkerRegisterRequest, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    row = db.query(WorkerNode).filter(
        WorkerNode.worker_id == req.worker_id).first()
    if not row:
    ...
    row.capabilities_json = json.dumps(sorted(set(req.capabilities)))
    row.status = "online"
    from datetime import datetime, timezone
...
def worker_heartbeat(worker_id: str, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    row = db.query(WorkerNode).filter(
        WorkerNode.worker_id == worker_id).first()
    if not row:
...
def report_worker_tool(worker_id: str, req: WorkerInstallReport, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if not worker:
        raise HTTPException(404, "Worker not registered")
    row = db.query(WorkerToolInstall).filter(WorkerToolInstall.worker_id ==
                                             worker_id, WorkerToolInstall.tool_id == req.tool_id).first()
    if not row:
        row = WorkerToolInstall(worker_id=worker_id, tool_id=req.tool_id)
        db.add(row)
    row.version = req.version
    row.state = req.state
    row.binary_path = req.binary_path
    row.checksum = req.checksum
    row.sbom_ref = req.sbom_ref
    from datetime import datetime, timezone
...
def worker_install_plan(worker_id: str, tool_id: str, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if not worker:
        raise HTTPException(404, "Worker not registered")
    tool = next((x for x in ai_ecosystem_registry()
                if x["id"] == tool_id), None)
    if not tool:
        tool = next((x for x in admin_tool_registry()
                    if x["id"] == tool_id), None)
    if not tool:
...
def v3_fabric_response_plan(req: V3ResponsePlanRequest, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    db.add(AuditEvent(actor=actor, action="v3_response_plan_staged",
           target=str(req.incident_id or "latest"), outcome="staged_only"))
    db.commit()
    return plan
...
def v31_trace_plan(req: V31TraceRequest, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    plan = build_trace_plan(req.source_ip, req.destination,
                            req.observed_at, req.indicators)
    user = _bearer_user(request)
    actor = user.username if user else "admin"
    db.add(AuditEvent(actor=actor, action="v31_adversary_trace_plan_staged",
           target=req.source_ip or req.destination or "unspecified", outcome=plan["plan_id"]))
    db.commit()
    return plan
...
def v39_rogue_agent_investigate_route(req: RogueAgentInvestigationRequest, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if result.get("status") != "not_found":
        user = _bearer_user(request)
        actor = user.username if user else "admin"
        db.add(AuditEvent(actor=actor, action="v39_rogue_agent_investigation",
               target=req.agent_id, outcome=result.get("case_id", "created")))
        db.commit()
    return result
...
def v40_tool_trust(tool_id: str, request: Request, db: Session = Depends(get_db)):
    _auth_user(request, db)
    releases = db.query(ToolRelease).filter(
        ToolRelease.tool_id == tool_id).all()
    score = 0
    factors = []
    if releases:
        ...
        if r.verification_status == "verified":
            score += 30
            factors.append("release metadata verified")
        if r.signature:
            score += 20
            factors.append("signature present")
        if r.provenance_uri:
            score += 15
            factors.append("provenance present")
        if r.sbom_uri:
            score += 15
            factors.append("SBOM present")
        if r.health_status == "healthy":
            score += 20
            factors.append("health gate passed")
...
def v40_register_release_route(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Release registration requires Sudo or Security Admin")
    try:
        return v40_register_release(db, payload)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
def v40_put_policy(tool_id: str, req: V40PolicyRequest, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Update policy changes require Sudo or Security Admin")
    if req.pin_mode not in {"floating", "pinned", "immutable"}:
        raise HTTPException(400, "Invalid pin_mode")
    if req.channel not in {"candidate", "canary", "stable", "extended_stable"}:
        raise HTTPException(400, "Invalid channel")
    from ..services.v40_tool_supply_chain import ensure_policy
    p = ensure_policy(db, tool_id)
    p.channel = req.channel
    p.pin_mode = req.pin_mode
    p.pinned_version = req.pinned_version
    p.pinned_digest = req.pinned_digest
    p.auto_update = req.auto_update
    p.critical_override = req.critical_override
    p.require_canary = req.require_canary
    p.canary_percent = req.canary_percent
    p.maintenance_window = req.maintenance_window
    db.commit()
    db.refresh(p)
    return clean(p)
...
def v40_freeze_tool(tool_id: str, payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    user = _bearer_user(request)
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(403, "Freeze requires Sudo or Security Admin")
    from datetime import datetime, timezone, timedelta
    from ..services.v40_tool_supply_chain import ensure_policy
    p = ensure_policy(db, tool_id)
    hours = float(payload.get("hours", 24))
    p.freeze_reason = str(payload.get("reason", "operator freeze"))
    p.frozen_until = datetime.now(
        timezone.utc)+timedelta(hours=max(.25, min(720, hours)))
    db.commit()
    return {"tool_id": tool_id, "status": "frozen", "frozen_until": p.frozen_until.isoformat(), "reason": p.freeze_reason}
...
def v40_quarantine_tool(tool_id: str, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    user = _bearer_user(request)
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(403, "Quarantine requires Sudo or Security Admin")
    db.query(ToolDefinition).filter(ToolDefinition.tool_id ==
                                    tool_id).update({"status": "quarantined"})
    db.query(ToolRelease).filter(ToolRelease.tool_id ==
                                 tool_id).update({"health_status": "quarantined"})
    db.commit()
    return {"tool_id": tool_id, "status": "quarantined", "execution": "blocked until reviewed"}
...
def v40_create_deployment(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    user = _bearer_user(request)
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(403, "Deployments require Sudo or Security Admin")
    try:
...
def v40_deployment_ack(deployment_id: str, payload: dict, request: Request, db: Session = Depends(get_db)):
    _auth_user(request, db)
    d = db.query(ToolDeployment).filter(
        ToolDeployment.deployment_id == deployment_id).first()
    if not d:
        raise HTTPException(404, "Deployment not found")
    receipt = payload or {}
    d.worker_receipt_json = json.dumps(receipt, default=str)
    d.state = str(receipt.get("state", "completed"))
    if d.state in {"completed", "healthy"}:
        from datetime import datetime, timezone
        d.completed_at = datetime.now(timezone.utc)
    db.commit()
    return {"deployment_id": deployment_id, "state": d.state, "receipt_sha256": sha256_json(receipt)}
...
def v40_rollback(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return v40_rollback_plan(db, payload["tool_id"], payload["worker_id"], payload["target_release_id"])
    except (KeyError, ValueError) as e:
        raise HTTPException(400, str(e))
...
def v40_health_check(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Health verdicts require Sudo or Security Admin")
    release_id = payload.get("release_id")
    release = db.query(ToolRelease).filter(
        ToolRelease.release_id == release_id).first()
    if not release:
        raise HTTPException(404, "Release not found")
    checks = payload.get("checks") or [{"name": "binary_integrity", "status": "pass"}, {"name": "dependency_integrity", "status": "pass"}, {
        "name": "safe_smoke_test", "status": "pass"}, {"name": "parser_regression", "status": "pass"}, {"name": "security_regression", "status": "pass"}]
    status = "healthy" if all(
        str(x.get("status")).lower() == "pass" for x in checks) else "failed"
    evidence = sha256_json({"release_id": release_id, "checks": checks})
    h = ToolHealthCheck(check_id="hc_"+uuid.uuid4().hex[:16], release_id=release_id, worker_id=payload.get(
        "worker_id", ""), status=status, checks_json=json.dumps(checks), evidence_sha256=evidence, notes=payload.get("notes", ""))
    release.health_status = status
    db.add(h)
    db.commit()
    db.refresh(h)
    if status == "failed":
        # artifact remains verified; deployment engine must not promote it
...
def v40_agent_swarm_containment(request: Request, db: Session = Depends(get_db)):
    ...
    if not p:
        p = AgentContainmentPolicy(policy_id="agent_default", name="Default Agent Containment", egress_mode="deny_by_default", allowed_destinations_json="[]",
                                   service_account_velocity_threshold=120, website_collaboration_detection=True, emergency_stop=False, require_human_approval_for_external_action=True)
        db.add(p)
        db.commit()
        db.refresh(p)
...
def v40_agent_swarm_containment_update(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Containment policy changes require Sudo or Security Admin")
    p = db.query(AgentContainmentPolicy).first()
    if not p:
        p = AgentContainmentPolicy(policy_id="agent_default")
        db.add(p)
    p.egress_mode = payload.get("egress_mode", p.egress_mode)
    p.allowed_destinations_json = json.dumps(payload.get(
        "allowed_destinations", json.loads(p.allowed_destinations_json or "[]")))
    p.service_account_velocity_threshold = int(payload.get(
        "service_account_velocity_threshold", p.service_account_velocity_threshold))
    p.website_collaboration_detection = bool(payload.get(
        "website_collaboration_detection", p.website_collaboration_detection))
    p.emergency_stop = bool(payload.get("emergency_stop", p.emergency_stop))
    p.require_human_approval_for_external_action = bool(payload.get(
        "require_human_approval_for_external_action", p.require_human_approval_for_external_action))
    db.commit()
    db.refresh(p)
    return {"policy": clean(p), "status": "updated"}


@router.post("/v40/agent-swarm/simulate")
def v40_agent_swarm_simulate(payload: dict, request: Request, db: Session = Depends(get_db)):
    _auth_user(request, db)
    actions = int(payload.get("actions_per_minute", 0))
    destinations = int(payload.get("external_destinations", 0))
    coordination = int(payload.get("coordination_sites", 0))
    privileged = bool(payload.get("privileged_identity", False))
    score = min(100, actions/2 + destinations*12 +
                coordination*15 + (25 if privileged else 0))
    controls = []
    if actions > 120:
        controls.append("freeze service identity")
    if destinations > 0:
        controls.append("deny unapproved egress")
    if coordination >= 2:
        controls.append("open swarm-correlation case")
    if privileged:
        controls.append("require privileged-session review")
...
def v40_register_artifact(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Artifact registration requires Sudo or Security Admin")
    if not payload.get("sha256") or len(payload["sha256"]) != 64:
        raise HTTPException(400, "sha256 must be 64 hex characters")
    a = ToolArtifact(artifact_id="art_"+uuid.uuid4().hex[:16], release_id=payload.get("release_id", ""), storage_uri=payload.get("storage_uri", ""), digest=payload.get("digest", "sha256:"+payload["sha256"]),
                     sha256=payload["sha256"], signature_status=payload.get("signature_status", "verified"), provenance_status=payload.get("provenance_status", "verified"), sbom_status=payload.get("sbom_status", "verified"), immutable=True)
    db.add(a)
    db.commit()
    db.refresh(a)
    return clean(a)
...
def v40_artifacts(request: Request, db: Session = Depends(get_db)):
    _auth_user(request, db)
    return [clean(x) for x in db.query(ToolArtifact).order_by(ToolArtifact.created_at.desc()).limit(200).all()]
...
def v42_attestation(release_id: str, payload: dict, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        return ingest_attestation(db, release_id, payload)
...
def v42_attestations(release_id: str, request: Request, db: Session = Depends(get_db)):
    _auth_user(request, db)
    rows = db.query(SupplyChainAttestation).filter(SupplyChainAttestation.release_id ==
                                                   release_id).order_by(SupplyChainAttestation.created_at.desc()).all()
    return [clean(x) for x in rows]
...
def v42_canary_plan(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return canary_plan(db, str(payload.get("tool_id", "")), str(payload.get("release_id", "")), list(payload.get("worker_ids") or []), int(payload.get("percent", 10)))
    except ValueError as exc:
        raise HTTPException(400, str(exc))
...
def v42_canary_evaluate(cohort_id: str, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return evaluate_canary(db, cohort_id)
    except ValueError as exc:
        raise HTTPException(400, str(exc))
...
def v42_trusted_promote(release_id: str, payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    try:
        return trusted_promote(db, release_id, str(payload.get("channel", "stable")))
    except ValueError as exc:
        raise HTTPException(400, str(exc))
...
def v42_register_ai_asset(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    row = db.query(AISupplyChainAsset).filter(
        AISupplyChainAsset.asset_id == payload["asset_id"]).first()
    if not row:
        row = AISupplyChainAsset(
            asset_id=payload["asset_id"], asset_type=payload["asset_type"], name=payload["name"], version=payload["version"])
        db.add(row)
    for k in ("digest", "source_uri", "provenance_uri", "sbom_uri", "policy_status", "trust_status"):
        if k in payload:
            setattr(row, k, str(payload[k]))
...
def v42_run_fixed_verifier(payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if verifier not in {"cosign", "syft", "grype", "tuf"}:
        raise HTTPException(400, "Verifier not allowlisted")
    if any(str(a).startswith("-") and a in {"--shell", "--command", "-c"} for a in args):
        raise HTTPException(400, "Shell execution flags are prohibited")
...
def v41_promote_release(release_id: str, payload: dict, request: Request, db: Session = Depends(get_db), _: bool = Depends(require_admin)):
    ...
    if user and user.role not in {"sudo", "security_admin"}:
        raise HTTPException(
            403, "Release promotion requires Sudo or Security Admin")
...
def v60_run_checklist(checklist_id: str, req: ChecklistRunRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if run.tier == "essential" and not req.approval_confirmed:
        raise HTTPException(
            403, "Essential checklists require approval_confirmed=true")
    ...
    db.add(AuditEvent(actor="console-user", action="checklist_run_requested",
           target=checklist_id, outcome=run.status))
    db.commit()
    ...
    try:
        ...
        prow = create_persisted_run(
            db, checklist_id, requested_by="console-user", parameters=req.parameters, notes=notes)
        # attach evidence if supplied
        if req.evidence:
            db.add(ChecklistResult(result_id="res_" + _uuid.uuid4().hex[:16], run_id=prow.run_id, checklist_id=checklist_id,
                   check_name="evidence", status="pass", evidence_json=json.dumps(req.evidence, sort_keys=True)))
            db.commit()
        ...
        # completed (stub) — no shell/network/DB writes beyond this record
        payload["status"] = prow.status
        # wire alerts (P6-G) — best-effort, never fails the run
        try:
            from app.services.alerts import notify_checklist_run

            notify_checklist_run(checklist_id, prow.run_id,
                                 run.tier, req.evidence)
        except Exception:
            pass
    except Exception:
        pass
...
def v60_get_run(run_id: str, db: Session = Depends(get_db)):
    ...
    results = db.query(ChecklistResult).filter(
        ChecklistResult.run_id == run_id).all()
    receipt = db.query(ChecklistReceipt).filter(
        ChecklistReceipt.run_id == run_id).first()
    return {
        "run_id": row.run_id,
...
def v60_get_receipt(run_id: str, db: Session = Depends(get_db)):
    ...
    row = db.query(ChecklistReceipt).filter(
        ChecklistReceipt.run_id == run_id).first()
    if not row:
...
class LiveIngestRequest(BaseModel):
    ...
    severity: str = Field(
        default="INFO", pattern="^(INFO|LOW|MEDIUM|HIGH|CRITICAL)$")
    payload: dict = Field(default_factory=dict)
...
def v61_ingest(req: LiveIngestRequest, db: Session = Depends(get_db), _auth: None = Depends(require_collector)):
    ...
    norm = normalize_event(req.sensor_type, req.event_type, req.payload,
                           req.sensor_id, req.severity, req.provenance, req.trace_id)
    row = persist_event(db, norm)
    db.add(AuditEvent(actor="sensor", action="live_sensor_ingested",
           target=req.sensor_type, outcome=req.severity))
    db.commit()
...
def v61_list_baselines(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    ...
    rows = db.query(NetworkBaseline).order_by(
        NetworkBaseline.created_at.desc()).limit(limit).all()
...
def v61_create_drop(req: DropCreateRequest, db: Session = Depends(get_db)):
    ...
    row = persist_drop(db, req.link_id, req.link_type, req.signal_dbm,
                       req.dhcp_state, req.dns_state, req.ap_assoc_state, req.recent_flaps)
...
def v61_list_drops(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    ...
    rows = db.query(DropEvent).order_by(
        DropEvent.created_at.desc()).limit(limit).all()
...
def domain_tools(domain_name: str, q: str | None = Query(None, max_length=128), limit: int = Query(200, ge=1, le=500)):
    ...
    slug_map = {d.lower().replace(" ", "-").replace("&", "").replace(",",
                                                                     "").replace("  ", "-"): d for d in DOMAINS_8}
    ...
    if label not in _D:
        raise HTTPException(
            404, f"Unknown domain '{domain_name}'. Expected one of {', '.join(_D)}")

    by = segregate_tools(all_tools())
    items = by[label]
    if q:
        ql = q.lower()
        items = [t for t in items if ql in t.get(
            "name", "").lower() or ql in t.get("category", "").lower()]
    items = items[:limit]
...
def agent_why_active(agent_id: str, db: Session = Depends(get_db)):
    ...
    has = db.query(AgentRuntimeEvent).filter(AgentRuntimeEvent.agent_id == agent_id).first() or db.query(AgentPolicy).filter(
        AgentPolicy.agent_id == agent_id).first() or db.query(PentestAgentPlan).filter(PentestAgentPlan.agent_id == agent_id).first()
    if not has:
        ...
        for base, _, files in _os.walk(root):
            for fn in sorted(files):
                ...
                try:
                    ...
                    pass
                items.append({"path": rel, "title": title, "section": rel.split(
                    "/")[0] if "/" in rel else "root"})
